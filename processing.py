...
if __name__ == '__main__':
    ...
    cout = 0
    # 数据路径
    data_path = "/home/lwc/data/第二批数据 不做滑窗"
    #e
    data_lists = get_filelist(data_path, [])
    # print(len(data_lists))
    min = 116

    data = np.zeros((len(data_lists), 1, 230, min, 1))
    ...
    np.save('不做滑窗/train_data.npy', train_data)
    np.save('不做滑窗/train_label.npy', train_label)
    np.save('不做滑窗/test_data.npy', test_data)
    np.save('不做滑窗/test_label.npy', test_label)
    np.save('不做滑窗/val_data.npy', val_data)
    np.save('不做滑窗/val_label.npy', val_label)
