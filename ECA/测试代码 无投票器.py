...
def a():
    ...
    net.load_state_dict(torch.load('/home/lwc/6-7/ECA/output/30TR20/30TR20_best_checkpoint.pth'))
    net.to(device)
    TS = 64  # 每个测试样本的投票数
    batch_size = 32
    test_data = torch.from_numpy(np.load('30TR20/test_data.npy')).float()
    test_label = torch.from_numpy(np.load('30TR20/test_label.npy')).float()
    ...
    np.savetxt("30TR20_accuracies.txt", accuracies)
    np.savetxt("30TR20_specificities.txt", specificities)
    np.savetxt("30TR20_sensitivities.txt", sensitivities)
