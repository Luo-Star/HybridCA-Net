...
from net.ResNet3D import resnet3d18
import logging
...
def train_model(model, train_loader, criterion, optimizer, num_epochs, log_filename, fold):
    ...
    for epoch in range(num_epochs):
        model.train()
        running_loss = 0.0
        for fmri_data, _, labels in train_loader:
            fmri_data, labels = fmri_data.to(device), labels.to(device)
            optimizer.zero_grad()
            outputs = model(fmri_data)
...
def validate_model(model, val_loader, criterion):
    ...
    with torch.no_grad():
        for fmri_data, _, labels in val_loader:
            fmri_data, labels = fmri_data.to(device), labels.to(device)
            outputs = model(fmri_data)
            loss = criterion(outputs, labels)
            val_loss += loss.item()
...
if __name__ == '__main__':
    ...
    for fold, (train_loader, val_loader) in enumerate(fold_dataloaders):
        resnet3d_model = resnet3d18(num_classes=len(class_names), dropout_prob=0.5).to(device)

        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(resnet3d_model.parameters(), lr=config['learning_rate'])

        logging.info(f"Fold {fold + 1}/{len(fold_dataloaders)}")
        train_model(resnet3d_model, train_loader, criterion, optimizer, num_epochs, log_filename, fold)
        val_loss, val_accuracy, sensitivity, specificity, roc_auc = validate_model(resnet3d_model, val_loader,
                                                                                   criterion)
        logging.info(f"Fold {fold + 1}: Validation Loss: {val_loss:.4f}, Validation Accuracy: {val_accuracy:.2f}%")
        logging.info(f"Fold {fold + 1}: Sensitivity: {sensitivity:.4f}, Specificity: {specificity:.4f}, ROC AUC: {roc_auc:.4f}")
