...
train_data = torch.from_numpy(np.load('AD-data/WD80/train_data_WD80.npy')).float()
train_label = torch.from_numpy(np.load('AD-data/WD80/train_label_WD80.npy')).long()  # 确保标签是长整型
val_data = torch.from_numpy(np.load('AD-data/WD80/val_data_WD80.npy')).float()
val_label = torch.from_numpy(np.load('AD-data/WD80/val_label_WD80.npy')).long()  # 确保标签是长整型
...
for epoch in range(200):  # number of epochs
    ...
    with torch.no_grad():
        ...
        if best_acc < val_accurate:
            ...
            torch.save(net.state_dict(), 'output/AD/WD80/AD_WD80_bs64.pth')
        ...
        print(results)
        with open('output/AD/WD80/AD_WD80_bs64.txt', 'a+') as f:
            time = datetime.now()
